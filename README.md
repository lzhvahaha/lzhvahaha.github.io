...
├── public/
│   └── pdfs/                   # ★ 论文与专利 PDF 原文件（Vite 原样复制到 dist/）
│       ├── 01-isolated-scan-stm.pdf            # 期刊论文 1
│       ├── ...                                 # 02~08 略
│       ├── 09-cryogenic-mfm-35t.pdf            # 期刊论文 9
│       ├── P01-opposing-motion-piezo-motor.pdf # 授权专利 1
│       └── ...                                 # P02~P06 略
└── src/
...
5. **PublicationsSection**（`#publications`）：标题 "Publications & Patents"，按 期刊论文（9 篇）/ 授权专利（6 项）两组展示，每条标题可点击在新标签页打开对应 PDF（论文 PDF 存放在 `public/pdfs/`）
...
`publicationsByYear` 数组，分两组：期刊论文（9 篇）+ 授权专利（6 项），每篇含 `title / authors / venue / pdf`：
- `venue` — 论文显示期刊名，专利显示状态（"已授权" / "实质审查"）
- `pdf` — 指向 `/pdfs/0x-xxx.pdf`（论文）或 `/pdfs/P0x-xxx.pdf`（专利）的路径

`PaperRow` 组件根据 `pdf` 字段判断：有 pdf 时标题渲染为 `<a target="_blank">` 链接，在新标签页打开 PDF；hover 时标题变为 `accent-blue` 色。专利与论文共用同一组件。分组标题用 `accent-blue` 色。
...
编辑 `src/components/PublicationsSection.jsx` 顶部的 `publicationsByYear` 数组，分为"期刊论文"（9 篇）与"授权专利"（6 项）两组。每篇含 `title / authors / venue / pdf`：
- 论文的 `venue` 为期刊名，专利的 `venue` 为状态
- `pdf` 字段指向 `public/pdfs/` 下对应的 PDF 文件路径（如 `/pdfs/01-isolated-scan-stm.pdf`）

### 改 / 新增论文或专利的 PDF

1. 将 PDF 文件放入 `public/pdfs/` 目录
   - 论文命名建议 `01-xxx.pdf` ~ `09-xxx.pdf`（数字前缀保持列表顺序）
   - 专利命名建议 `P01-xxx.pdf` ~ `P06-xxx.pdf`（P 前缀区分专利）
2. 在 `PublicationsSection.jsx` 对应条目中填写 `pdf: '/pdfs/文件名.pdf'`
3. 标题会自动变成可点击链接，新标签页打开 PDF
4. 不需要 PDF 的条目省略 `pdf` 字段即可，标题保持纯文本
